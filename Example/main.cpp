...
const std::string DFLT_SERVER_ADDRESS{"mqtts://192.168.11.121:8883"};
const std::string DFLT_CLIENT_ID{"cpp-mqtt"};
const std::string DFLT_USERNAME{""};
const std::string DFLT_PASSWORD{""};
...
const std::string TOPIC{"Hello12345"};
                             ...
                             public virtual mqtt::iaction_listener
{
      ...
      void connected(const std::string &cause) override
      {
            ...
            std::cout << "\n[" << std::put_time(&tm, "%H:%M:%S") << "] "
                      << "Sending  message with QOS=" << QOS << " on topic 'Hello12345'... "
                      << std::endl;

            auto msg = mqtt::make_message(
                "Hello12345", "Cedalo is awesome! (paho c++ client v3.1.1)",
                QOS, false);
            mqtt::delivery_token_ptr tok = cli_.publish(msg);
            tok->wait_for(100); // just slow down for demo
            ...
            subtok->wait_for(100); // just slow down for demo
